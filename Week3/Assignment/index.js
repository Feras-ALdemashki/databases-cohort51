...
async function findEpisodesExercises(client) {
  ...
  const resultEpisode2 = await client
    .db("databaseWeek3")
    .collection("bob_ross_episodes")
    .findOne({ episode: "S02E02" });
  console.log(`The title of episode 2 in season 2 is ${resultEpisode2.title}`);
  ...
  const resultCliffEpisode = await client
    .db("databaseWeek3")
    .collection("bob_ross_episodes")
    .find({ elements: "CLIFF" })
    .toArray();
  const cliffEpisode = resultCliffEpisode.map((e) => e.title);
  console.log(`The episodes that Bob Ross painted a CLIFF are ${cliffEpisode}`);
  ...
  const CliffLighthouseEpisodes = await client
    ...
    .toArray();
  const titles = CliffLighthouseEpisodes.map((e) => e.title);
  console.log(
...
async function updateEpisodeExercises(client) {
  ...
  console.log(
    `Ran a command to update all the BUSHES to BUSH and it updated ${result_1.modifiedCount} episodes`
  );
}
